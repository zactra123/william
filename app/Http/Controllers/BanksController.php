...
class BanksController extends Controller
{
    ...
    public function showBankLogo(Request $request)
    {
        if (!is_null($request->term)) {
            ...
            $banksLogos = BankLogo::leftJoin(
                "bank_addresses",
                "bank_logos.id",
                "=",
                "bank_addresses.bank_logo_id"
            )
                ->leftJoin(
                    "equal_banks",
                    "bank_logos.id",
                    "=",
                    "equal_banks.bank_logo_id"
                )
                ->where(function ($query) use ($request, $phoneFaxZip, $name) {
                    $query
                        ->whereRaw(
                            "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(bank_logos.name, ',', ''), '.', ''), '\'', ''),'\"', ''),' ','') LIKE '%{$name}%'"
                        )
                        ->orWhereRAW(
                            "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(`bank_addresses`.`name`, ''), ',', ''), '.', ''), '\'', ''),'\"', ''),' ','') LIKE '%{$name}%'"
                        )
                        ->orWhereRAW(
                            "CONCAT(COALESCE(bank_addresses.street, ''), ' ', COALESCE(bank_addresses.city, ''), ' ', COALESCE(bank_addresses.state, '')) LIKE '%{$request->term}%'"
                        )
                        ->orWhereRAW(
                            "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(`equal_banks`.`name`, ''), ',', ''), '.', ''), '\'', ''),'\"', ''),' ','') LIKE '%{$name}%'"
                        );
                    if ($phoneFaxZip) {
                        $query = $query
                            ->orWhereRaw(
                                "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(bank_addresses.phone_number, ',', ''), '.', ''), '(', ''), ')', ''), ' ', ''), '-', '') LIKE '%{$phoneFaxZip}%'"
                            )
                            ->orWhereRaw(
                                "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(bank_addresses.fax_number, ',', ''), '.', ''), '(', ''), ')', ''), ' ', ''), '-', '') LIKE '%{$phoneFaxZip}%'"
                            )
                            ->orWhereRaw(
                                "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(bank_addresses.zip, ',', ''), '.', ''), '(', ''), ')', ''), ' ', ''), '-', '') LIKE '%{$phoneFaxZip}%'"
                            );
                    }
                });
            $banksLogos->get();
            if (!is_null($request->states)) {
                $banksLogos = BankLogo::where(
                    "bank_addresses.type",
                    "executive_address"
        ...
        if (!is_null($request->types)) {
            ...
            $banksLogos = $banksLogos->where(function ($query) use (
                $types,
                $non_b_sba,
                $b_sba
            ) {
                ...
                if ($non_b_sba !== false) {
                    $query = $query->orWhereRaw(
                        "bank_logos.additional_information LIKE '%NON-BANK SBA LENDER%'"
                    );
                }
                if ($b_sba !== false) {
                    $query = $query->orWhereRaw(
                        "bank_logos.additional_information LIKE '%BANK-SBA LENDER%'"
                    );
                }
            });
        }
        ...
        if (!empty($request->character)) {
            if ($request->character == "#") {
                $banksLogos = $banksLogos->whereRaw(
                    "TRIM(LOWER(name)) NOT REGEXP '^[a-z]'"
                );
            } else {
                $banksLogos = $banksLogos->whereRaw(
                    "LOWER(`name`) LIKE '{$request->character}%'"
                );
            }
        }

        $banksLogos = $banksLogos
            ->groupBy("bank_logos.id")
            ->select("bank_logos.*")
            ->orderBy("bank_logos.name")
            ->paginate(20);
    ...
    public function bankMissingFields(Request $request)
    {
        $banksLogos = BankLogo::leftJoin(
            ...
            ->where("bank_addresses.type", "executive_address")
            ->whereRaw(
                "bank_logos.type != 5 AND (bank_addresses.name IS NULL OR bank_addresses.name = '' OR bank_addresses.phone_number IS NULL OR bank_addresses.phone_number ='')"
            );
        if (!is_null($request->term)) {
            $phoneFaxZip = preg_replace("/[^0-9a-zA-Z]/", "", $request->term);
            $name = str_replace([" ", ",", ".", '\'', '"'], "", $request->term);
            ...
            $banksLogos->get();
            if (!is_null($request->states)) {
                $banksLogos = $banksLogos->where(
                    "bank_addresses.state",
                    $request->states
                );
            }
        } else {
    ...
    public function show($id)
    {
        ...
        $registredAgent = BankLogo::where("type", "registered_agent")
            ->pluck("name", "id")
            ->toArray();

        return view(
            "furnishers._show_bank",
            compact(
                "bank",
                "account_types",
                "bank_addresses",
    ...
    public function store(Request $request)
    {
        ...
        if ($validation->fails()) {
            ...
            return redirect()
                ...
                ->with(
                    "error",
                    "Your name field is empty please add data in it!"
                );
        }
        ...
        if (!empty($request["logo"])) {
            ...
            $bankLogoExtension = strtolower(
                $imagesBankLogo->getClientOriginalExtension()
            );
            if (!in_array($bankLogoExtension, $imageExtension)) {
                return redirect()
                    ...
                    ->with(
                        "error",
                        "Please upload the correct file format (PDF, PNG, JPG)"
                    );
            }
            ...
            $pathLogo = $request
                ->file("logo")
                ->storeAs("bank_logos", "furnisher_$time.$ext", [
                    "disk" => "s3",
                    "visibility" => "public",
                ]);
        }
        ...
        if (
            !in_array($bank->type, [2, 55]) ||
            (empty($bank->additional_information["sub_type"]) ||
                !in_array(
                    "MORTGAGE",
                    $bank->additional_information["sub_type"]
                ))
        ) {
            unset($account_addresses["qwr_address"]);
        }
        $additional_addresses = !empty($account_addresses["additional_address"])
            ? $account_addresses["additional_address"]
            : false;
    ...
    public function edit(Request $request)
    {
        ...
        $addresses = $bank->bankAddresses()->get();
        $bank_addresses = [];
    ...
    public function update(Request $request)
    {
        ...
        if (
            !empty($request->bank["additional_information"]["collection_type"])
        ) {
            $bank_additonal_information = $bank->toArray()[
                "additional_information"
            ];
            $bank_additonal_information["collection_type"] =
                $request->bank["additional_information"]["collection_type"];
    ...
    public function banKName(Request $request)
    {
        ...
        if ($keywordId != null) {
            $accType = AccountTypeKeyWord::where(
                "account_type_key_id",
                $keywordId
            )
                ->pluck("account_type_id")
                ->toArray();
            $account_types = AccountType::whereIn("id", $accType)
                ->pluck("name", "id")
                ->toArray();
        } else {
            $account_types = AccountType::where("type", true)
                ->pluck("name", "id")
                ->toArray();
        }
        $data = $account_types;
        ...
        $banks = BankLogo::leftJoin(
            ...
            ->where(function ($query) use ($request, $phoneFaxZip, $name) {
                $query
                    ...
                    ->orWhereRAW(
                        "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(bank_addresses.name, ',', ''), '.', ''), '\'', ''),'\"', ''),' ','') LIKE '%{$name}%'"
                    )
                    ->orWhereRAW(
                        "CONCAT(COALESCE(bank_addresses.street, ''), ' ', COALESCE(bank_addresses.city, ''), ' ', COALESCE(bank_addresses.state, '')) LIKE '%{$request->search_key}%'"
                    )
                    ->orWhereRAW(
                        "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(equal_banks.name, ',', ''), '.', ''), '\'', ''),'\"', ''),' ','') LIKE '%{$name}%'"
                    );
                if ($phoneFaxZip) {
                    $query = $query
                        ->orWhereRaw(
    ...
    public function types(Request $request)
    {
        if ($request->isMethod("post")) {
            ...
            $check_type = AccountType::firstWhere(
                "name",
                $request->account_type["name"]
            );
            if ($check_type) {
                // return redirect()->route('admins.bank.types')
                //     ->withErrors("FURNISHER TYPE ALREADY EXIST");
                return redirect()
                    ->route("admins.bank.types")
                    ->with("error", "Furnisher Typr Already Exist!");
            }
    ...
    public function update_type_keywords(Request $request)
    {
        ...
        foreach ($request->keywords as $key) {
            $keyword = AccountTypeKeys::firstorCreate([
                "key_word" => strtoupper($key),
            ]);
    ...
    {
        $result = AccountTypeKeys::where(
            "account_type_keys.key_word",
            "LIKE",
            "%{$request->search_key}%"
        )->get(["id", "key_word"]);
    ...
    public function mortgageDays(Request $request)
    {
        if ($request->type == "non_judicial") {
            $states = State::where("type", 1)
                ->orderBy("name")
                ->get();
        } elseif ($request->type == "judicial") {
            $states = State::where("type", 2)
                ->orderBy("name")
                ->get();
        } else {
        ...
        $stateArr = State::select(
            DB::raw('CONCAT(full_name, "(",name,")") AS name'),
            "id"
        )
            ->pluck("name", "id")
            ->toArray();
    ...
    public function changeAddress()
    {
        $creditUnion = BankLogo::leftJoin(
            ...
            ->whereRaw(
                "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(bank_addresses.name, ',', ''), '.', ''), '\'', ''),'\"', ''),' ','') LIKE '%POBOX%'"
            )
        ...
        $federalCreditUnion = BankLogo::leftJoin(
            "bank_addresses",
            "bank_logos.id",
            "=",
            "bank_addresses.bank_logo_id"
    ...
    public function changeDisputeAddress($union, $type)
    {
        foreach ($union as $id) {
            ...
            if (
                !is_null($disChange->name) &&
                !preg_match('/(\sCU|\sFCU)$/', $disChange->name)
            ) {
                $executive = BankAddress::where(
                    "bank_logo_id",
                    $disChange->bank_logo_id
                )
                    ->where("type", "executive_address")
                    ->first();
                ...
                if (is_null($executive->name)) {
                    ...
                    $name = BankLogo::whereId(
                        $disChange->bank_logo_id
                    )->first();
                    if ($type = "55") {
                        $exChange["name"] = str_replace(
    ...
    public function med_serv()
    {
        ...
        foreach ($data as $k => $med) {
            ...
            $bank = BankLogo::leftJoin(
                ...
                ->pluck("bank_logos.name", "bank_logos.id")
                ->toArray();
