				...
				<aside class="sidebar col-md-3 p-0">
					<div class="card ">
						<div class="card-body">
							<div>
								<div class="pb-1">
									<span class="btn text-primary">{{ zactra::translate_lang('Write new message') }}</span>
									<form method="POST" action="{{ route('admin.message.store') }}">
										@csrf
										<div class="form-group row m-1">
											<input id="first_name" type="text" class="form-control" name="full_name" value="{{ old('full_name') }}" required autocomplete="full_name" placeholder="{{ zactra::translate_lang('First and Last name') }}">
										</div>
										<div class="form-group row m-1">
								...
								<div class="col-md-12 ">
									<div class="row p-2 ">
										<div class="list-group list-group-horizontal col-md-6">
											<a class="list-group-item list-group-item-action p-1 tab-selector active" href="{{route("admin.message.index")}}">{{ zactra::translate_lang('All Messages') }}</a>
											<a class="list-group-item list-group-item-action p-1 tab-selector pending" href="{{route("admin.message.index", ["type" => "pending"])}}">{{ zactra::translate_lang('Pending') }}</a>
											<a class="list-group-item list-group-item-action p-1 tab-selector completed" href="{{route("admin.message.index", ["type" => "completed"])}}">{{ zactra::translate_lang('Completed') }}</a>
										</div>
										<div class="col-md-6 ">
									...
									<table class="table table-hover">
										<thead>
											<tr>
												...
												<th scope="col">{{ zactra::translate_lang('Phone number') }}</th>
												<th scope="col">{{ zactra::translate_lang('Call date') }}</th>
												<th scope="col">{{ zactra::translate_lang('Status') }}</th>
												<th scope="col">{{ zactra::translate_lang('Details') }}</th>
											</tr>
										</thead>
										<tbody>
  											...
  											<tr>
  												...
  												<td>
  													@if($message->completed == 0)
    													<button class="btn btn-success" id="message-completed" data-target={{$message->id}}>
    														<span class="fa fa-check"></span>
    													</button>
    													<meta name="csrf-token" compelted-token="{{ csrf_token() }}">
  													@endif
  													<button class="btn btn-secondary modalViewMessage" data-toggle="modal" role="button" data-target={{$message->id}}>
  														<span class="fa fa-file"></span>
  													</button>
  													<button class="btn btn-secondary modalAddNote" data-toggle="modal" role="button" data-target={{$message->id}}>
...
<div class="modal fade" id="addNotes" tabindex="-1" role="dialog" aria-labelledby="favoritesModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title" id="favoritesModalLabel">{{ zactra::translate_lang('Add note') }}</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">Close</span> </button>
			</div>
			<div class="modal-body">
				<form method="POST" action="{{ route('admin.message.note') }}">
					...
					<div class="form-group row mb-0">
						<div class="col-md-9 offset-md-5">
							<button type="submit" class="btn btn-primary">
								{{ zactra::translate_lang('Add note') }}
							</button>
						</div>
					</div>
				</form>
