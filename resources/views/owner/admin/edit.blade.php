    ...
    <div>
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ zactra::translate_lang('Edit') }}</li>
            </ol>
        </nav>
    </div>
</div>
            ...
            <div class="card">
                <div class="card-body">
                    <a href="{{ route('owner.admin.index') }}">
                        <h5 class="text-dark"><i class="ti-angle-left"></i>{{ zactra::translate_lang('Edit Admin') }}</h5>
                    </a>
                    {!! Form::open(['route' => ['owner.admin.update', $admin->id], 'method' => 'POST', 'class' => 'mt-5 m-form m-form label-align-right admin-form', 'novalidate'=> 'novalidate']) !!} @method('PUT') @csrf
                    ...
                    <div class="form-group row font" id="delete-{{$value->id}}">
                        <div class="col-sm-12 col-12 col-md-11 col-lg-11 form-group">
                            {{Form::hidden('admin[ip_address]['.$value->id.'][id]', $value->id)}} {{ Form::text('admin[ip_address]['.$value->id.'][ip_address]', $value->ip_address, ['class' => 'form-control', 'placeholder'=>'IP Address',
                            'required']) }}
                        </div>
                    ...
                    <div class="form-group row mb-0 font pull-right">
                        <div class="col-md-12">
                            <input type="submit" class="btn btn-primary" value="{{ zactra::translate_lang('Update') }}" class="ms-ua-submit" />
                        </div>
                    </div>
                    {!! Form::close() !!}
                </div>
