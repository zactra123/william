...
//es em avelacrel
var $correct_full_name = /^[a-z]([-']?[a-z]+)*( [a-z]([-']?[a-z]+)*)+$/gi;
  ...
  if (this.validType == "input") {
    ...
    if (this.type == "text" && this.name == "full_name") {
      if (this.val != "" && this.val.length > $minLengthTel && this.$correct_full_name.test(this.val)) {
        this.input.siblings(".required").hide("fast", function () {
          $(this).remove();
        });
        this.input.removeClass("error").addClass("not-error");
        if (this.input.siblings(".wpcf7-not-valid-tip")) {
          this.input.siblings(".wpcf7-not-valid-tip").slideUp("fast");
        }
      } else {
        this.input.removeClass("not-error").addClass("error");
      }
      ...
      if (this.val != "" && this.$length.test(this.val) && this.$upperCase.test(this.val) && this.$digit.test(this.val) && this.$symbol.test(this.val)) {
        this.input.removeClass("error").addClass("not-error");
      } else {
        this.input.removeClass("not-error").addClass("error");
      }
...
function ValidOnSubmit($form, e) {
  ...
  if ($formHasError) {
    ...
    $(".submit-required").hide("fast", function () {
      $(this).remove();
    });
